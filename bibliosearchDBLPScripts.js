...
function initialize(){
	...
	//sessionStorage.setItem("BiblioSearch-User_Selection-DBLP", JSON.stringify(selectionData));
		...
		obtainResultsDBLP(searchMethod, searchTerm, desiredResults);
	}
...
async function obtainResultsDBLP(searchMethod, searchTerm, desiredResults){
    ...
    try{
		...
		if(searchMethod.includes("/api")){
			while (fetchedResults.length < desiredResults){
				let obtained = await searchDBLP(generateNewAPIRequest(searchMethod, searchTerm, desiredResults, fetchedResults.length)).then((e) => {return e;});
				if(obtained == undefined){
					throw new Error("No results received")
				}
				let elements = parser.parseFromString(obtained,"text/xml");
				//console.log(elements);
				data = elements.getElementsByTagName("hit");
				if(data.length == 0){
					throw new Error("No results available for the specified search")
				}
				fetchedResults = fetchedResults.concat(Array.from(data));
				if(fetchedResults.length>=elements.getElementsByTagName("hits")[0].getAttribute("total")){
					break;
				}
			}
		}
			...
			let suffix = "";
			if(searchMethod == "db/"){suffix = "/index"};
			let obtained = await searchDBLP(generateNewRequest(searchMethod, searchTerm, suffix, defaultFormatDBLP)).then((e) => {return e;});
			if(obtained == undefined){
				throw new Error("No results receied")
			}
			//let jsobj = x2j.xml_str2json(obtained);
			//console.log(jsobj);
			//return;
			let elements = parser.parseFromString(obtained,"text/xml");
			...
			if(searchMethod == "db/" && term.includes("/conf/") && liElems.length>0){
				for (let liElem of liElems){
					//results.forEach(async function(result){
						...
						if(refList.length>0){
							if (refList[0].hasAttribute("href")){
								let urlPre = refList[0].getAttribute("href");
								urlPre = urlPre.replace(".html","");
								let urlSplit = urlPre.split("/");
								urlValue = urlSplit[1];
								for(let i = 2; i<urlSplit.length; i++){
									urlValue+="/";
									urlValue+=urlSplit[i];
								}
								let volumeObtained = await searchDBLP(generateNewRequest(searchMethod, urlValue, "", defaultFormatDBLP)).then((e) => {return e;});
								if(volumeObtained==undefined){
									//error
									obtained=="";
								}
								else{
									let volumeElements = parser.parseFromString(volumeObtained,"text/xml");
									data = data.concat(Array.from(volumeElements.querySelectorAll("article, proceedings, inproceedings, book")));
								}
							}
			...
			/*else if(searchMethod == "db/" && term.includes("/journals/") && data.length>0){
